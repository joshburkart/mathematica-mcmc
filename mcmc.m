...
Clear[TestMCMCInput];
TestMCMCMFInput[data_List, errors_List, model_, paramspec_, vars_List, num_Integer] :=
(
	...
	If[!If[# == 0, 1, n]&[Length[data[[1,1]]]] == If[# == 0, 1, n]&[Length[vars]],
...
MCMCModelFit::nonnumer = "Model supplied does not evaluate to a number for initial parameters; abort.";
...
MCMCModelFit[data_List, errors_List, model_, paramspec_, invars_, num_Integer, opts : OptionsPattern[]] /;
	...
	Module[{chisq, plog, params, spreads, state, stateval, sets, Ns, discrete, 
		...
		bestfitparams, z, corr, vars, plogfunc},
				...
				status = OptionValue["ProgressMonitor"] /. \
					{
						...
						"Plot" -> If[Length[vars] == 1 && And[! FreeQ[OptionValue["ProgressMonitor"], "Plot"], i > 2],
							Show[
